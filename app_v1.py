...
tz_info = st.session_state.utc_time.tzinfo
#st.write(f" tz info: {tz_info}")
#lt_now=utc_time2.astimezone()
#st.write(f" lt now: {lt_now}")
#st.write("difference: "+st.session_state.utc_time.strftime("%z"))
...
#st.write(aware_us_central.strftime("%z"))
local_time_now=st.session_state.utc_time.astimezone(pytz.timezone("Africa/Windhoek"))
#st.write(local_time_now)
#
...
for tname in timezone_list:
    #tzone = pytz.timezone(tname)
    #timethere=st.session_state.utc_time.astimezone(pytz.timezone(tname))
    offset_string=st.session_state.utc_time.astimezone(pytz.timezone(tname)).strftime("%z")
    offset_float=float(offset_string)/100
    #st.write(f" {tname} {timethere}" +timethere.strftime("%z"))
    #print('{n} UTC{z}'.format(n=tname, z=std_date.strftime('%z')))
    if offset_float not in offset_list_float:
    	#b=float(offset_string)/100
    	#offset_list_string.append(offset_string)
    	offset_list_float.append(offset_float)


#st.write(offset_list_string)
#offset_list_float=[]
#for a in offset_list:
	#b=float(a)/100
	#if b not in offset_list_float:
		#offset_list_float.append(b)


offset_list=[]
offset_list_float_sorted=sorted(offset_list_float)
#st.write(offset_list_float_sorted)
for offset_float in offset_list_float_sorted:
	t="UTC "+("{:+.2f}".format(offset_float)).replace(".",":")
	#o=str(offset_float)
	#if o<0:
	#	offset="UTC "+str(o)
	#else:
...
if "show_local_time" not in st.session_state:
	st.session_state.show_local_time=False
if "timezone_option" not in st.session_state:
	st.session_state.timezone_option="by Name"
...
row2_col1,row2_col2,row2_col3,row2_col4,row2_col5,row2_col6=st.columns([5,6,6,2,6,6])	
...
with row5_col1:
	...
	st.checkbox("Show Local Time", value=False, key="show_local_time", on_change=None)
...
with row5_col3:
	...
	#st.write(str(st.session_state.leapseconds))
	#placeholder_localtime=st.empty()
	#placeholder_localtime.radio("Select Timzone",('by UTC offset', 'by Name'))
	#if st.session_state.show_local_time:
	#	local_time_now=st.session_state.utc_time.astimezone(pytz.timezone("Africa/Windhoek"))

	#	placeholder_localtime.write(f"Local Time: {local_time_now}")
...
row6_col1,row6_col2,row6_col3=st.columns([12,1,12])	
with row6_col1:
	placeholder_localtime=st.empty()
	if st.session_state.show_local_time:
		local_time_now=st.session_state.utc_time.astimezone(pytz.timezone("Africa/Windhoek"))
...
row7_col1,row7_col2,row7_col3,row7_col4=st.columns([7,9,1,12])	

with row7_col1:
	placeholder_radiobutton=st.empty()
	if st.session_state.show_local_time:
		placeholder_radiobutton.radio("Select Timezone",('by Name','by UTC offset'),key="timezone_option")
		#st.write(st.session_state.timezone_option)
	else:
		placeholder_radiobutton.write("")
with row7_col2:
	placeholder_selectbox=st.empty()
	if st.session_state.show_local_time:
		if st.session_state.timezone_option=="by Name":
			st.selectbox("Timezone", timezone_list, key="local_timezone_name")
		else:
			st.selectbox("Timezone", offset_list, key="local_timezone_offset")
...
tab1, tab2 = st.tabs(["UTC Time", "Local Time"])

with tab1:
	st.subheader("UTC Time")
with tab2:
	st.subheader("Local Time")
	g=st.time_input("Hours : Minutes", value=st.session_state.utc_time, key=None )
