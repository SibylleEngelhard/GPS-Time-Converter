...
def update_utc_and_to_gps():
    ...
    st.session_state.display_hour=st.session_state.display_time.hour
    st.session_state.display_minute=st.session_state.display_time.minute
    st.session_state.display_second=st.session_state.display_time.second
...
def update_gps_and_to_utc():
    ...
    st.session_state.leapseconds=calc_leapseconds_from_gpstime(st.session_state.gps_time)
    st.session_state.utc_time=st.session_state.gps_time-datetime.timedelta(seconds=st.session_state.leapseconds)
    s.session_state.display_time=st.session_state.utc_time.astimezone(pytz.timezone(st.session_state.selected_timezone))
    st.session_state.display_day=st.session_state.display_time.day
    st.session_state.display_month=st.session_state.display_time.strftime("%B")
    st.session_state.display_year=st.session_state.display_time.year
    st.session_state.display_hour=st.session_state.display_time.hour
...
with row0_col1:
    radiobuttontime=st.radio("Convert to/from:", ("UTC","Local Time"),key="selected_time", on_change=change_time, horizontal=True)
...
with row3_col2:
    st.text(" ")
    st.text(" ")
    st.button("Now",key="but1",help="settings of your PC clock",on_click=update_display_time_now)
with row3_col3:
    st.text(" ")
    st.text(" ")
    st.button("0:00:00",key="but2",on_click=update_display_time_zero)
with row3_col5:
...
st.write("UTC:  "+str(st.session_state.utc_time))
if st.session_state.show_local_time:
    st.write("Display Time:  "+str(st.session_state.display_time))  
